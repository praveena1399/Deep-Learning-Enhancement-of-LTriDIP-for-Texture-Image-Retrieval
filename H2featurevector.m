function f = H2featurevector(image)
img=im2double(image);
...
for ii=2:m-1
 for jj=2:n-1
    p1=0; 
    for su=1:8
              p1=p1+abs(img(ii,jj)-img(ii+ss(su),jj+tt(su)));
     end
    m2=0.125*(p1);
    x=1;
    t=1;
    ...
    for u=0:7
       LTriDP1(ii,jj)=LTriDP1(ii,jj)+((2^u)*z2(u+1));
       LTriDP2(ii,jj)=LTriDP2(ii,jj)+((2^u)*z3(u+1));
       LTriDP3(ii,jj)=LTriDP3(ii,jj)+((2^u)*mag(u+1));
     end
  end
 end
